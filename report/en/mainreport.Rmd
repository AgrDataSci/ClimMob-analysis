...
Figure 1.1 shows the same information in Table 1.1 with the number of valid answers received by each trait in each data collection moment in this project. 
...
Figure 1.2 shows that the `r length(items)` `r options` are all connected to each other. That means that they all co-occurred at least once in an incomplete block of `r ncomp` `r options`. 
...
```{r plsplot, dpi=dpi,fig.height=6, fig.width=6, fig.cap= paste0("Figure 1.4. Partial least squares biplot of relationship between the tested ", options, " and the traits evaluated.")}
if (isTRUE(dim(arrows)[[1]] > 0)) {
