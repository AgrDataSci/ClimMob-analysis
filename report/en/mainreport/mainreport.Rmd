...
pls_summary_line <- paste("")

if (isTRUE(dim(arrows)[[1]] > 0)) {
  pls_summary_line <- paste("Partial least squares regression was used to determine", 
                            "relationship between the specific characteristics and", 
                            "the overall performance. The first two components", 
                            "recombining the specific characteristics",
                            "are able to explain", paste0(round(yve[2] * 100, 2), "%"),
                            "of the variability in the overall performance.",
                            "The dashed line represents the overall performance,", 
                            ...
                            "in different characteristics. Better performance in",
                            "characteristics will correspond with arrows pointing",
...
if (isTRUE(length(tree_f) > 1)){
  ...
  node_statement <- paste("Table 1.5 shows which", options,
                          ...
                          "subgroups identified by the Plackett-Luce tree for", 
...
if (isFALSE(is.na(ps[1]))) {
    ...
    if (isTRUE(ps[1] < sig_level)) {
      stat_diff_statement <- paste0("There were statistically significant", 
                                    ...
                                    options ," in the overall performance (p = ",
...
if (any(na.omit(ps[2:length(ps)]) < sig_level)){
    stat_diff_statement <- paste0(stat_diff_statement,
                                 ...
                                 paste(na.omit(trait_full[ps[2:length(ps)] < sig_level]), collapse=", "))
}
...
if (isTRUE(length(na.omit(outtabs[[1]]$p.value)) > 0)) {
  if (isTRUE(any(na.omit(outtabs[[1]]$p.value) < sig_level))) {
    effect_covar_statement <- paste("Statistically significant relationship to the overall", 
                                    "performance was found for", length(siglist), 
                                    "of the covariates tested. The significant relationship", 
                                    "was found for:", 
                                    paste(paste0(siglist," (p = ",outtabs[[1]][siglist,"p"],")."),
                                          collapse=", "))
  }
  
  if(isFALSE(any(na.omit(outtabs[[1]]$p.value) < sig_level))) {
    effect_covar_statement <- paste0("None of the covariates tested were found to have a",
                                     "statistically significant relationship to the overall 
                                     performance at the ", expression(alpha), " = ", 
                                     sig_level, ".")
  }
}
...
no_tree <- ""
if (isTRUE(length(tree_f)==1)) {
  no_tree <- "No split was found for the Plackett-Luce model using the selected covariates."
}

# if more then one characteristic then a paragraph for the relationship between 
# characteristics is added

## Relationships between characteristics
relation_other_chrs1 <- "This report only assessed the 'overall performance'."
...
relation_other_chrs1 <- 
  paste0("Table 1.6 shows, for each characteristic in the project, ", 
        "the frequency for which the rankings matched with the overall ", 
        "performance. Best and worst agreement represents the percentage ", 
        "for which the best and worst ", option, " for the characteristic ", 
        "matched the overall best and worst. Complete ranking agreement ", 
        "shows the proportion of correlation on the full ranking with the ", 
        "overall performance as baseline using the Kendall correlation ", 
        "coefficient [@Kendall1938].")

relation_other_chrs2 <- 
  paste0("The characteristic which had the strongest ", 
        "relationship with the overall performance was ", strongest_link[1], 
        ". Overall the rankings for ",  strongest_link[1], 
        " matched the rankings for the overall performance ", strongest_link[2],
        "% of the time. The characteristic which had the weakest ", 
        " relationship with the overall performance was ",
        weakest_link[1], ". Overall the rankings for " , weakest_link[1], 
        " matched the rankings for the overall performance only ",
        weakest_link[2], "% of the time. The correlation with the other ", 
        "characteristics is also shown in Figure 1.2.")

}

fav_other_chars <- "This report only assessed the 'overall performance'."
...
cont_other_chars <- "This report only assessed the 'overall performance'."
...
plmodel_other_chars <- "This report only assessed the 'overall performance'."
if (isTRUE(length(other_traits) > 0)) {
  plmodel_other_chars <-
  paste0("This section shows the Plackett-Luce models without covariates", 
         " built using the rankings for each of the ",
         length(other_traits), " other ", 
         "characteristics that were evaluated by the ", rankers, 
         " in this project.")
}

```

You are reading a report generated by ClimMob, a software to design, manage, analyze trial data generated by crowdsourced citizen science.
...
Overall there were `r nranker` `r rankers` contributing to this project. Each `r ranker` assessed `r ncomp` different `r options` and ranked them in order of its overall performance. In addition they also provided rankings for `r nothertraits` additional characteristics: 
...
Table 1.2 shows the `r options` assessed within this project, with the frequency and percentage of `r rankers` who assessed each `r option`. If the `r options` has large names (> 10 characters) an abbreviation was applied across the figures in this report.
...
A summary of the p-values testing the hypothesis that there exist differences in the rankings within each of the Plackett-Luce models fitted for each of the assessed characteristics, and the list of `r options` which were significantly highest and lowest ranked overall, are summarised in Table 1.3.
...
A summary of the univariate significance levels for all covariates that were able to be tested is shown in Table 1.4.
...
## Correlation between the overall performance and the other characteristics

`r paste(relation_other_chrs1)`

```{r agreement_table}
if (length(other_traits) > 0) {
kable(agreement_table,
      caption = "Table 1.6. Correlation between the 'overall performance' and the others characteristics assessed in this report.")
}
```

\

`r paste(relation_other_chrs2)`


```{r correspondence, dpi=dpi,fig.height=agreem_h, fig.width=9, fig.cap="Figure 1.2. Correlation between individual characteristic and the 'overall performance'."}
if (length(other_traits) > 0) {
  plot(agreement) +
  theme_minimal() +  
...
```{r pls_plot, dpi=dpi,fig.height=6, fig.width=6, fig.cap="Figure 1.3. Partial least squares biplot of relationship between other characteristics and overall performance."}
if (isTRUE(dim(arrows)[[1]] > 0)) {
  plot(pls_plot)
}
```
...
knitr::kable(fav2,
             row.names = FALSE,
             caption = "Table 2.1. Favourability scores for overall performance.")
```
...
The `r option` `r fav1[[1,1]]` was the 'best' `r option` overall being ranked highest by `r round(fav1[[1,3]], 1)`% of the `r nranker` `r rankers` who assessed this `r option`. 

\

### Other characteristics  

`r paste(fav_other_chars)`
  ...
  for(i in seq_along(fav_other_traits)){
    trait_summaries<-c(trait_summaries,
                       knitr::knit_child(paste0(fullpath, "/report/",
                                                language, "/mainreport/favourability_analysis_other_traits.Rmd"), 
                                         quiet=TRUE))
}  
}
...
```{r contests_bars, dpi=dpi, fig.height=contest_h, fig.width=9, fig.cap="Figure 2.2. Head to head performance for 'overall performance'."}
if(length(other_traits) > 0){
  plot(contests[[2]]) + 
  labs(y = "", x = "") + 
  theme_minimal() +  
...
if (isTRUE(length(other_traits_list) > 0)) {
  for (i in seq_along(other_traits_list)) {
    model_traits <- c(model_traits,
                      knitr::knit_child(paste0(fullpath, "/report/", 
                                               language, "/mainreport/aov_other_traits.Rmd"), 
                                        quiet = TRUE))
}
}
...
Figure 5.1 shows the partitioning of the rankings based on the most significantly different sub-groups which could be identified from the data using a `r sig_level*100`% significance level. At the top of the tree is the full dataset, then working down through the different levels of the tree shows the combinations of variables which define each subgroup. The model parameters are shown for the final subgroups ("terminal nodes") in the plots at the bottoms of the tree. The model quasi-variance estimates, along with `r ci_level*100`% confidence intervals are provided. This will help identification of which `r options` were better suited to particular sub-groups identified by the analysis.

```{r pltree, dpi=500, asis=TRUE,fig.height=8,fig.width=10, fig.cap="Figure 5.1. Plackett-Luce tree for the overall performance. The horizontal axis is the probability of winning. Error bars show quasi-SEs. The gray vertical line indicates the average probability of winning (1/number of items)."}
gosset:::plot_tree(tree_f, add.letters = TRUE)
```
...
```{r node_summary1}
if (length(siglist) > 0) {
  table2stat <- paste("The highest and lowest performing", option, "within each sub-group is identified within Table 5.2.")
}else{
  table2stat <- ""
}
```
