...
if (isTRUE(dim(arrows)[[1]] > 0)) {
  pls_summary_line <- paste("Partial least squares regression was used to determine relationship between the specific characteristics and the overall performance.",
                            "The first two components recombining the specific characteristics",
...
if (isTRUE(length(tree_f) > 1)){
  ...
  node_statement <- paste("Table 1.2.2 shows which", options,
...
if (any(na.omit(ps[2:length(ps)]) < sig_level)){
    stat_diff_statement <- paste0(stat_diff_statement,". Statistically significant differences were also",
...
if (isTRUE(length(na.omit(outtabs[[1]]$p.value)) > 0)) {
  ...
  if(isFALSE(any(na.omit(outtabs[[1]]$p.value) < sig_level))) {
    effect_covar_statement <- paste0("None of the covariates tested were found to have a statistically significant ",
                                     "relationship to the overall performance at the ", expression(alpha), " = ", 
                                     sig_level, ".")
  }
}
...
A summary of the univariate significance levels for all covariates that were able to be tested is shown in Table 1.2.1.
...
kable(agreement_table,
      caption = "Table 2.5. Relationship between individual characteristic and the 'overall performance'.")
```

\

```{r correspondence, dpi=dpi,fig.height=agreem_h, fig.width=9, fig.cap="Figure 2.4. Agreement between individual characteristic and the 'overall performance'."}
if (length(other_traits) > 0) {
  plot(agreement) +
  theme_minimal() +  
  theme(axis.text.x = element_text(size = 10, color = "#000000"),
...
```{r pls_plot, dpi=dpi,fig.height=6, fig.width=6, fig.cap="Figure 2.5. Partial least squares biplot of relationship between other characteristics and overall performance."}
if (isTRUE(dim(arrows)[[1]] > 0)) {
  plot(pls_plot)
}
```
