---
title: "`r paste('Report of ClimMob project ', project_name)`"
author: "ClimMob.net"
...
Overall, there were `r nparticipants`  registered to this project. Each `r participant` assessed `r noptions` different `r options` and provided rankings for `r ntraits` trait(s). 
...
The rate of response in each trial stage (data collection moment) is presented in Figure 1.1. This gives an overview of the level of participation during the experiment and any attrition (when participants leave the experiment). 
...
if (isTRUE(trial_map$geoTRUE)) {
knitr::include_graphics(trial_map$map_path)
}
```
...
```{r network, dpi=dpi, fig.height=7, fig.width=7, fig.cap=paste0("Figure 1.2. Experimental network representation of ", options, " tested in this trial.")}
