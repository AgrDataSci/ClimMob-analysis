...
if (length(trait_dropped) > 0) {
  ...
  trait_note <- paste0("The", adj, paste(trait_dropped, collapse = ", "), verb, "not considered in this report because", noun, "did not surpassed the thresholds of (1) having at least ", missper, " valid entries, and (2) that all the ", options, " are tested at least twice per given trait. Table 1.1. shows the traits analysed in this report.")
}
```
...
if (covarTRUE) {
  if (isTRUE(length(covar_dropped) > 0)) {
    drop_covar_statement <- paste0(
      ...
      "' were not used in the analysis, because they did not surpassed ",
...
if (length(tree_f) > 1) {
  pltree_text <- paste0(pltree_text, " Nodes split by the following covariate and attribute ", paste(paste0("node ", node_ids, " covariate ", tolower(node_summary[,1]), " attribute: " , gsub('["]', "", tolower(node_summary[,2]))), collapse = "; and "), ".")
}
