...
However, since this report was done in an automated process, is likely that it will not provide to you the full overview on the performance of the tested technologies as the automation may not capture all the factors that influence the performance of a technology in the target environment (under real-world conditions). If you would like to have an in-deep analysis, or to merge data from different projects or is planning a peer-review publication with your tricot data, send a proposal/request to the ClimMob Team <climmob@mrbotcr.com>. 
...
```{r map, dpi=dpi, out.width = '70%', fig.cap=paste0("Figure 2. Map of trial locations. Administrative boundaries are based on the Database of Global Administrative Areas (GADM). The red–blue dots indicate the geographic distribution of trials implemented within the study.")}
if (isTRUE(trial_map$geoTRUE)) {
  plot(trial_map$map)
}
```
