...
if (isPLADMM) {
  relation_other_chrs3 <- 
  paste0("Additionally, we used the Alternating Directions Method of Multipliers (ADMM) algorithm proposed by Yildiz (2020) [@Yildiz2020] which models the log-worth as a linear function of item covariates. ",
         "In this report, the log-worth of the ", nothertraits, " other traits were used as a item covariate,  or the intrinsic characteristics of the tested ", options, " to check  which trait (if any) has a significant influence to the rankings of the reference trait (", ovname, ").", 
...
```{r correspondence, dpi=dpi,fig.height=agreem_h, fig.width=12, fig.cap= paste0("Figure 1.3. Kendall tau correlation between traits and ", Ovname, ".")}
if (nothertraits > 0) {
  pagreement
}
```
...
```{r est_plot, dpi=dpi, fig.height=favplot_h, fig.width=5, fig.cap=paste0("Figure 2.2. Plackett-Luce Model estimates (log-worth) of tested ", options, " for '", Ovname,"' with ", ci_level*100, "% confidence intervals. Different letters indicate significant differences at p < ", sig_level ,". The ", option, " ", reference, " is set as reference (log-worth arbitrarily set to zero).")}
...
covar_statement <- ""
if (covarTRUE) {
  covar_statement <- paste0("You provided the covariate(s): ", paste3(sort(covar$codeQst), lan = language), ". ")
...
if (!isTREE) {
  no_tree <- 
    ...
    "covariates. You can refer to Figure 2.2 to check the performance of the tested ", options, " in this experiment.")
}
```
...
if(isTRUE(nothertraits > 0)) {
  ...
  fav_traits[[1]] <- fav_traits[[1]][-reference_trait]
  fav_traits[[2]] <- fav_traits[[2]][-reference_trait]
