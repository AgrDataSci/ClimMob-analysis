...
if (isPLADMM) {
  relation_other_chrs4 <- 
  paste0("Additionally, the Alternating Directions Method of Multipliers (ADMM) algorithm [@Yildiz2020] was applied to each model log-worth as a linear function of item covariates. ",
  ...
  if (length(rmcor) > 2) {
    relation_other_chrs4 <- paste0(relation_other_chrs4, 
                                   " The traits ", paste3(otn[rmcor], lan = language), " were not used in this analysis due to multicollinearity.")
  }
...
```{r agreementplot, dpi=dpi,fig.height=agreem_h, fig.width=agreem_h, fig.cap= paste0("Figure 1.3. Kendall tau correlation between traits and ", Ovname, ".")}
if (nothertraits > 0) {
  pagreement
}
```
...
```{r worthmap, dpi=dpi,fig.height=agreem_h, fig.width=agreem_h, fig.cap= paste0("Figure 1.4. Worth parameters (probability of winning) by traits of ", options, " tested in this experiment. The chart highlights the ", options, " with performance above a cut-off point of 1/N items.")}
if (length(worthmap) > 0) {
  worthmap
}
```
