#' Organize the tricot ranking data from a ClimMob project
#' 
#' Prepares data using internal parameters and
#' the parameters sent by ClimMob to get the data 
#' ready for analysis 
#' 
#' @param pars a json list with parameters sent by ClimMob
#' @param cmdata an object of class CM_list with ClimMob data
...
#' @param reference_tech a vector with integer indicating the reference technology(ies)
#' @param tech_index a vector of characters for the indices in cmdata
#' @param minN minimum n of complete data required in a trait evaluation before it is excluded
#' @param minitem minimum n of items tested in the available data
#' @param mincovar minimum proportion of covariates compared to total valid n
#' @param sig_level significance level for the Plackett-Luce model
#' @examples 
...
#'                                  pars = pars, 
#'                                  groups = "gender1", 
#'                                  option_label = "variety",
#'                                  ranker_label = "farmer",
#'                                  reference_tech = c(1, 2, 4),
...
organize_ranking_data = function(cmdata, 
  ...
  reference_tech = unlist(lapply(reference_tech, function(x) x$alias_name))
