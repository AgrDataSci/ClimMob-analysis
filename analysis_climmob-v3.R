...
#I keep it for latter, because I don't know the struture of this object
####
...
for(i in seq_along(nameschars)){
  ...
  # make sure that rankings are ordered from best to worst
  if (ncomp == 3) {
    bestindex <- which(grepl("_pos$", names(rank_i)))
    worstindex <- which(grepl("_neg$", names(rank_i)))
    
    rank_i <- rank_i[c(bestindex, worstindex)]
    
  }
  
  # get the items
  items_i <- data[, paste0("item_",LETTERS[1:ncomp])]
